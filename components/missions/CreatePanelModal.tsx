...
interface CreatePanelModalProps {
  isOpen: boolean;
  onClose: () => void;
  onCreatePanel: (userId: string, sendTo: string[], receiveFrom: string[]) => void;
}

export function CreatePanelModal({ isOpen, onClose, onCreatePanel }: CreatePanelModalProps) {
  const [newUserId, setNewUserId] = useState('');
  const [selectedSendTo, setSelectedSendTo] = useState<string[]>([]);
  const [selectedReceiveFrom, setSelectedReceiveFrom] = useState<string[]>([]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onCreatePanel(newUserId.trim(), selectedSendTo, selectedReceiveFrom);
    handleClose();
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 transition-opacity">
      <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-md m-4 transform transition-all">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-bold text-gray-800">Create New Panel</h2>
        ...
        <form onSubmit={handleSubmit}>
          <div className="mb-4">
            <label htmlFor="panel-user-id" className="block text-sm font-medium text-gray-700 mb-1">
              New Panel ID
            </label>
            <input
              type="text"
