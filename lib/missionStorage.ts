...
export function getMissionsForPanel(
  ...
  const normalizePoint = (point: string | null | undefined): string | null => {
  ...
  return validMissions
    .map((mission) => {
      ...
      if (mission.status === 'Completed' && mission.startPoint && mission.destination) {
        const startPoint = normalizePoint(mission.startPoint);
        const destinationPoint = normalizePoint(mission.destination);
        ...
        // Show in activating panel (the one that clicked Send/Receive)
          ...
          const ownsStart = startPoint ? panelSelectedAreas.includes(startPoint) : false;
          const ownsDestination = destinationPoint ? panelSelectedAreas.includes(destinationPoint) : false;
          const canReceiveFromStart = startPoint ? panelReceiveFrom.includes(startPoint) : false;
          const canSendToDestination = destinationPoint ? panelSendTo.includes(destinationPoint) : false;
          
          if (ownsStart || canReceiveFromStart) {
            // Panel received this mission → show as Receive
            displayType = 'Receive';
            shouldShow = true;
          } else if (ownsDestination || canSendToDestination) {
      ...
      // For Pending missions, use location-based filtering
        ...
        if (mission.type === 'Send' && mission.destination) {
          ...
          // If another panel created it and this panel can Receive From that destination, show as Receive
            ...
            !isMissionAssigned &&
            destinationPoint &&
            (panelSelectedAreas.includes(destinationPoint) || panelReceiveFrom.includes(destinationPoint))
          ) {
            displayType = 'Receive';
            shouldShow = true;
          }
          ...
          // If another panel created it and this panel can Send To that start point, show as Send
            ...
            !isMissionAssigned &&
            startPoint &&
            (panelSelectedAreas.includes(startPoint) || panelSendTo.includes(startPoint))
          ) {
            displayType = 'Send';
            shouldShow = true;
          }
